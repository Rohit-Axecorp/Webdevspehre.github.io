...
export default function Counter() {
  ...
  return (
    <div className="bg-gray-100 2xl:py-12 py-8">
      <div className="container mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 text-center">
        {stats.map((stat, index) => (
          ...
          >
            {/* Animated Number */}
            <p className="text-3xl sm:text-5xl 2xl:text-7xl font-bold text-[#ED1E3A]">
              <CountUp end={stat.number} duration={3} />+
            </p>

            {/* Label */}
            <p className="mt-4 text-sm 2xl:text-lg font-medium text-gray-600">{stat.label}</p>
          </div>
        ))}
      </div>
    </div>
